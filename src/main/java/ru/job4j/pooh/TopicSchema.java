...
public class TopicSchema implements Schema {
    ...
    public void run() {
        while (!Thread.currentThread().isInterrupted()) {
            for (var queKey : receivers.keySet()) {
                var queue = data.getOrDefault(queKey, new LinkedBlockingQueue<>());
                var receiversByQueue = receivers.get(queKey);
                for (var receiver : receiversByQueue) {
